...
ll depth[MAX_N+1]; int height[MAX_N+1];
void dfs(int prev, int v, ll h, int c) {
    first[v] = ++cnt;
    up[v][0] = prev;
    depth[v] = h; height[v] = c;
...
void solve() {
    ...
    while (Q--) {
        ...
        if (mod == 1) {
            ...
            int u = lca(v, w);
            if (height[v]-height[u] < k) {
                k = (height[v]-height[u])+(height[w]-height[u])-k;
                swap(v, w);
            }
            cout << kth(k, v, u) << '\n';
        }
